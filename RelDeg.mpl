...
if % = lasterror then printf("WARNING current working point can not be substitu\
ted in the (internal) computations of the relative degree but the computations \
are proceeded until a fatal ERROR will occur \n "); elif % = 0 then def := '
false'; notdefseq := notdefseq, j; else def := 'true'; break; end if; end if; 
end do; if not def then notdef := notdef, i; notdefseq := [notdefseq]; for kk 
to nops(notdefseq) do solseq := lasterror; if solseq <> lasterror then for jj 
to nops(solseq) do condhlp := NULL; for kk to nops(op(jj,solseq)) do if not op(
1,op(kk,op(jj,solseq))) = op(2,op(kk,op(jj,solseq))) then condhlp := condhlp, 
...
"The decoupling matrix is: \n"); print(nprintf(`#msup(mi("A"),mn("<%d>"))`,
DEAstep)(nprintf(`#msup(mi("x"),mn("<%d>"))`,DEAstep)) = eval(Adeg)); printf(
"      \n"); printf("      \n"); printf("      \n"); end if; if 0 < nops({
notaff}) then rddef := 'false'; printf("The relative degree cannot be defined f\
or this system, the outputs below are not directly affected by the input. \n ")
; printf(
" The rank of the decoupling matrix is less than the number of output(s). \n");
...
ber of outputs, %a. Therefore, the relative degree is not defined at x0. \n",m)
...
'false'; printf("Although each output has a well defined relative degree, but t\
he rank of the decoupling matrix is less than the number of outputs \n "); 
zinput := NULL; for i to m do zc[i] := NULL; for j to p do zc[i] := zc[i], lgh[
j,i]; end do; if {zc[i]} = {0} then zinput := zinput, i; end if; end do; 
printrdeg := not_defined; reldeg := not_defined; Adegnull := map(simplify,map(
normal,subs(xwork,op(Adeg)))); numofNonZe := 0; io := Vector(m); jo := Vector(m
); for j to m do for i to m do if Adegnull[i,j] <> 0 then numofNonZe := 
numofNonZe+1; io[numofNonZe] := i; jo[numofNonZe] := j; end if; end do; end do;
elif lgh = n and locrtot = n and reldeg = NULL then printf("The total relative \
degree at the working point is equal to the number of states. \n"); end if; 
printf("r-indices are given by: "); print(r^cat(`<`,DEAstep,`>`) = eval(locrdeg
^%T)); if nops({notdef}) <> 0 or nops({notaff}) <> 0 then printf("      \n"); 
printf("      \n"); printf("      \n"); end if; if Determinant(map(simplify,map
(normal,subs(xnull,lgh)))) = 0 and nops({notdef}) = 0 and nops({notaff}) = 0 
then printf("The number(s) between [] give the vector and total relative degree\
 of each seperate output, not_defined means that this relative degree is not pr\
operly defined. \n"); printf("      \n"); printf("      \n"); printf("      \n"
